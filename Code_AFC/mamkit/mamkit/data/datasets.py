...
class MMUSEDFallacy(Loader):
    ...
    def __init__(
        ...
        # self.archive_url = 'https://zenodo.org/api/records/15183217/files-archive'
    ...
    def _get_text_data(
        ...
        if self.task_name == 'afc':
            ...
            inputs = inputs[mask]
            labels = labels[mask]
            context = context[mask] if context is not None else None
    ...
    def _get_text_audio_data(
        ...
        if self.task_name == 'afc':
            ...
            mask = pd.notna(labels)
            texts = texts[mask]
            text_context = text_context[mask] if text_context is not None else None
            audio = audio[mask]
            audio_context = audio_context[mask] if audio_context is not None else None
            labels = labels[mask]
