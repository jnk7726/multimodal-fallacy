...
from mamkit.modules.transformer import MulTA_CrossAttentionBlock, MulTA_SelfAttentionBlock, MulTA_CrossAttentionBlock_VISUAL
...
class MulTA_JointAttention(TextAudioModel):
    """
    Modified MulTA model with joint self-attention over concatenated text and audio features.
    """
    ...
    def __init__(
            ...
            positional_encoder,
            audio_dropout_rate=0.1,
            text_dropout_rate=0.1  # <- keep original params
    ):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.d_ffn = d_ffn
        ...
        # Use either one of the dropouts (or average, or pick)
        dropout_rate = (audio_dropout_rate + text_dropout_rate) / 2

        self.joint_attention_blocks = th.nn.ModuleList([
            MulTA_SelfAttentionBlock(self.embedding_dim, self.d_ffn, dropout_prob=dropout_rate)
            for _ in range(self.n_blocks)
        ])
        self.positional_encoder = positional_encoder()
        
        self.segment_embed = th.nn.Embedding(2, self.embedding_dim)
    ...
    def forward(self, inputs):
        ...
        # Concatenate features and attention masks along sequence dimension
        combined_features = th.cat([text_features, audio_features], dim=1)  # [batch, seq_text + seq_audio, emb_dim]
        ...
        segment_ids = th.cat([
        th.zeros(seq_len_text, dtype=th.long, device=text_features.device),
        th.ones(seq_len_audio, dtype=th.long, device=audio_features.device)
                            ], dim=0).unsqueeze(0).expand(batch_size, -1)  # [batch, seq_total]
        segment_embeddings = self.segment_embed(segment_ids)  # [batch, seq_total, emb_dim]
        
        combined_features = combined_features + segment_embeddings
        
        
        combined_attentions = th.cat([text_attentions, audio_attentions], dim=1)  # [batch, seq_text + seq_audio]

        # Pass through joint self-attention blocks
        out = combined_features
        for block in self.joint_attention_blocks:
            out = block(out, combined_attentions)

        # Pooling (mean across sequence)
        pooled = th.mean(out, dim=1)  # [batch, emb_dim]
...
#changed
class MulTA_VISUAL(TextAudioModel):
    """
    Class for the unaligned multimodal model
    """
    ...
    def __init__(
        ...
        self.head = head()
        self.text_crossmodal_blocks = th.nn.ModuleList([
            MulTA_CrossAttentionBlock_VISUAL(self.embedding_dim, self.d_ffn, dropout_prob=text_dropout_rate) for _ in
            range(self.n_blocks)
        ])
        self.audio_crossmodal_blocks = th.nn.ModuleList([
            MulTA_CrossAttentionBlock_VISUAL(self.embedding_dim, self.d_ffn, dropout_prob=audio_dropout_rate) for _ in
            range(self.n_blocks)
        ])
        self.positional_encoder = positional_encoder()
    ...
    def forward(
        ...
        text_crossmodal_out = text_features
        for cm_block in self.text_crossmodal_blocks:
            text_crossmodal_out, _ = cm_block(text_crossmodal_out, audio_features, audio_attentions, return_attn=True)
