...
class MulTA_SelfAttentionBlock(torch.nn.Module):
    """
    Transformer encoder block (self-attention + feedforward) for joint attention over combined features.
    """

    def __init__(self, embedding_dim, d_ffn, num_heads=4, dropout_prob=0.1):
        super().__init__()
        self.layer_norm = torch.nn.LayerNorm(embedding_dim)
        self.mh_attention = torch.nn.MultiheadAttention(embed_dim=embedding_dim, num_heads=num_heads,
                                                        dropout=dropout_prob, batch_first=True)
        self.pointwise_ff = PositionwiseFeedForward(d_model=embedding_dim, hidden=d_ffn)

    def forward(self, x, attn_mask):
        """
        Args:
            x: [batch, seq_len, emb_dim] combined input
            attn_mask: [batch, seq_len] mask (1=valid, 0=pad)
        """
        x_norm = self.layer_norm(x)
        attn_mask_float = attn_mask.to(torch.float32)

        # Self-attention: query=key=value=x_norm
        mh_out, _ = self.mh_attention(x_norm, x_norm, x_norm, key_padding_mask=attn_mask_float, need_weights=False)
        add_out = mh_out + x  # Residual

        add_out_norm = self.layer_norm(add_out)
        ff_out = self.pointwise_ff(add_out_norm)
        out = ff_out + add_out  # Final residual
        return out

#changed
class MulTA_CrossAttentionBlock_VISUAL(torch.nn.Module):
    def __init__(self, embedding_dim, d_ffn, num_heads=4, dropout_prob=0.1):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.d_ffn = d_ffn
    ...
    def forward(self, elem_a, elem_b, attn_mask, return_attn=False):
        elem_a = self.layer_norm(elem_a)
        elem_b = self.layer_norm(elem_b)
        attn_mask = attn_mask.to(torch.float32)

        mh_out, attn_weights = self.mh_attention(elem_a, elem_b, elem_b, key_padding_mask=attn_mask, need_weights=True)
        add_out = mh_out + elem_a
        add_out_norm = self.layer_norm(add_out)
        out_ffn = self.pointwise_ff(add_out_norm)
        out = out_ffn + add_out
