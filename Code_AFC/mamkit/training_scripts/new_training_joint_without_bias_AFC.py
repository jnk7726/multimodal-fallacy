...
from mamkit.models.text_audio import MulTA, MulTA_JointAttention
from mamkit.utility.callbacks import PycharmProgressBar
from mamkit.utility.model import MAMKitLightingModel
from mamkit.data.datasets import MultimodalDataset

# Mapping from class label (int) to its bias tokens
per_class_banned_tokens = {
    0: {"people", "american", "america", "president", "us", "united", "states", "##s", "world"},
    1: {"people", "american", "president", "senator", "mr", "uh", "us", "country", "##s"},
    2: {"president", "senator", "mr", "uh", "people", "clinton"},
    3: {"president", "people", "americans", "america", "administration", "bill", "year"},
    4: {"president", "people", "america", "united", "states", "government", "##s"},
    5: {"america", "president", "change", "great", "promise", "law", "order", "person"},
}

def remove_bias_tokens(dataset, bias_map):
    texts, audio, labels, text_ctx, audio_ctx = [], [], [], [], []
    
    for ex in dataset:
        text, audio_path, label, text_context, audio_context = ex
        
        if label is not None and label in bias_map:
            banned_tokens = bias_map[label]
            words = text.split()
        ...
        labels.append(label)
        text_ctx.append(text_context)
        audio_ctx.append(audio_context)
...
if __name__ == '__main__':
    ...
    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'mmused-fallacy', 'afc', 'text_audio_multa')
    ...
    config = MulTAConfig.from_config(key=ConfigKey(dataset='mmused-fallacy',
                                                   input_mode=InputMode.TEXT_AUDIO,
                                                   task_name='afc',
                                                   tags={'anonymous'}))
    ...
    for seed in config.seeds:
        ...
        for split_info in loader.get_splits(key='mancini-et-al-2024'):
            ...
            split_info.train = remove_bias_tokens(split_info.train, per_class_banned_tokens)
